...
db_full4new_stim_screen_pupil_select1_scr$median_split_valence_ground<- cut(db_full4new_stim_screen_pupil_select1_scr$ValenceMean, 2)


db_full4new_stim_screen_pupil_select1_scr$median_split_valence_self<- cut(db_full4new_stim_screen_pupil_select1_scr$valence, 2)
db_full4new_stim_screen_pupil_select1_scr %>%
 subset(arousal_outler == FALSE)%>%
       subset(BIO_CDA.SCR_outl  == FALSE)%>%
  subset(BIO_CDA.PhasicMax!= 0)%>%
  group_by(ssid,median_split_valence_self)%>%
  ...
  group_by(ssid, Group,median_split_valence_self)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE)%>%
  ggplot(aes(AQ, cor_scr_ar3,color = Group))+
  geom_point(aes(color = Group))+
  geom_smooth(method = lm, se = F)+
  facet_grid(~median_split_valence_self)+
  ggpubr::stat_cor()
...
hr_plots$tas_hrdif<- hr_plots$tas_hrdif+
  # scale_y_continuous(limits = c(-10,-2))+
  scale_x_continuous(breaks = c(-2, -1, 0, 1), limits = c(-2, 1.2))
...
hr_plots$AQ_hrdif<- hr_plots$AQ_hrdif+
  # scale_y_continuous(limits = c(-10,-2))+
  scale_x_continuous(breaks = c(-2, -1, 0, 1), limits = c(-2, 1.2))
...
```{r}
# descriptices mean and sd by high and low ASD
library(tidyverse)
for_demog_new<- db_full6new_hr_fix_stim_select3_no_outl %>%
  group_by(ssid,Group)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE)%>%
  ungroup()%>%
  # group_by(Group)%>%
  # summarise_if(is.numeric, mean, na.rm = TRUE)%>%
  select(c(ssid,Group, TAS, AQ,log1p_BIO_CDA.PhasicMax,Bio_Mean_HR_dif, pup_basCor, arousal, valence))


# grouped%>%
#   summarise(grouped, mean=mean(value), sd=sd(value))

unique(demog_summ$ssid)

demog_summ %>%
  select(c(ssid, Group, TAS, AQ, DASSTotal,BIO_CDA.PhasicMax,Bio_Mean_HR_dif, pup_basCor))
  
colnames(demog_summ)
demog_summ$WASIM
for_demog_new2<- left_join(for_demog_new, demog_summ[,c(1,13,25)])

write_csv(for_demog_new2, "for_demog_new2.csv")

for_demog_new3<- subset(for_demog_new2, ssid!= 600)
for_demog_new4<- subset(for_demog_new3, Age< 51)

write_csv(for_demog_new4, "for_demog_new5.csv")
colnames(for_demog_new4)
psych::describe.by(for_demog_new4[,3:11], for_demog_new4$Group)
```
