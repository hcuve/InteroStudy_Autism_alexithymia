...
?TInPosition::tepBADA.inference.battery
var.BADA.inf <- TInPosition::tepBADA.inference.battery(var.data, 
                                                       scale = TRUE, 
                                                       center = TRUE, 
                                                       DESIGN = var.design, 
...
db_forBADA2<- data.table::dcast(setDT(db_full6new_hr_fix_stim_select5), 
                    ssid + Group ~ stimIAPS2, value.var = c('pup_basCor',
...
var.design_asd <- as.factor(all.data_asd$Group)
...
factoextra::fviz_nbclust(scaled_all.data_asd2pcs_forkmean, 
                         kmeans, 
                         method='silhouette')
...
clusplot(scaled_all.data_asd2pcs_forkmean,
         y_kmeans,
         lines = 2,
         shade = TRUE,
         color = TRUE,
...
kmeans$tot.withinss/kmeans$totss
...
table(is.finite(as.data.frame(var.data_asd)))\\?TExPosition::print.tepDICA

?TExPosition::tepDICA

infinite_issue<-var.data_asd[,colSums(var.data_asd != 0) != 0]

table(is.infinite(scale(var.data_asd)))

var.data_asd_sca<- scale(var.data_asd)
var.data_asd_sca<- as.data.frame(var.data_asd_sca)
var.data_asd_sca
var.data_asd_sca[var.data_asd_sca == "NaN"]
na.omit(var.data_asd_sca)

var.data_asd_sca%>%
  mutate(colSumst= colSums())
  subset(colSums!= 0)

  
var.data_asd_sca[, colSums(var.data_asd_sca != 0) > 0]
  

Sample.scaled.2 

data.frame(t(na.omit(t(var.data_asd_sca))))
(var.data_asd)

?svd
svd(var.data_asd_sca)

log(var.data_asd_sca+10)
var.BADA.inf_asd_doica <- TExPosition::tepDICA(var.data_asd_sca+10,
                                                       # scale = TRUE, 
                                                       # center = TRUE, 
                                                       DESIGN = as.factor(all.data_asd$Group), 
                                                       make_design_nominal = TRUE, 
                                                       group.masses = NULL, 
                                                       weights = NULL, 
                                                       symmetric = TRUE,
...
resBADA.inf <- TInPosition::tepBADA.inference.battery(var.data+10, scale = FALSE,
                  DESIGN = var.design,
                  test.iters = 100,
                  graphs = FALSE)
